...
# Video file to save locally
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
local_video_path = f"twitch_stream_{timestamp}.mp4"
video_writer = cv2.VideoWriter(local_video_path,
...
if __name__ == "__main__":
    ...
    # Release local video writer
    video_writer.release()

    # Upload the recorded video to S3
    s3_key = f"videos/{os.path.basename(local_video_path)}"
    try:
        print("Uploading video now...")
        s3.upload_file(local_video_path, bucket_name, s3_key)
        print(f"✅ Uploaded to S3: {s3_key}")
        os.remove(local_video_path)
        print(f"🗑️ Deleted local file: {local_video_path}")
    except Exception as e:
